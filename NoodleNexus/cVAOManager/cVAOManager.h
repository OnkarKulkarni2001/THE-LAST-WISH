...
struct sVertex_SHADER_FORMAT_xyz_rgb_N_UV
{
	...
	float nx, ny, nz;	// in vec3 vNormal;
	float u, v;			// in vec2 vUV;
	float tx, ty, tz;   // in vec3 vTangent;  // Add tangent
	//float bx, by, bz;   // in vec3 vBitangent; // Add bitangent
};
...
private:
	void ComputeTangentsAndBitangents(sModelDrawInfo& modelDrawInfo);
	// Called by c'tor
