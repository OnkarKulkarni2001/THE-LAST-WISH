	...
	unsigned int shaderProgramID)
{
	...
	GLint vtangent_location = glGetAttribLocation(shaderProgramID, "vTangent");    // Add tangent
	GLint vbitangent_location = glGetAttribLocation(shaderProgramID, "vBitangent"); // Add bitangent
	...
	glVertexAttribPointer(vtangent_location, 3, GL_FLOAT, GL_FALSE,
		sizeof(sVertex_SHADER_FORMAT_xyz_rgb_N_UV),
		(void*)offsetof(sVertex_SHADER_FORMAT_xyz_rgb_N_UV, tx));

	// Bitangent attribute
	//glEnableVertexAttribArray(vbitangent_location);
	//glVertexAttribPointer(vbitangent_location, 3, GL_FLOAT, GL_FALSE,
	//	sizeof(sVertex_SHADER_FORMAT_xyz_rgb_N_UV),
	//	(void*)offsetof(sVertex_SHADER_FORMAT_xyz_rgb_N_UV, bx));
	...
	unsigned int shaderProgramID)
{
	...
	// Compute tangents and bitangents after loading the vertex data
	ComputeTangentsAndBitangents(drawInfo);
