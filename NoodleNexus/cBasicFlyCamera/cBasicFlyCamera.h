...
public:
	...
	float boundaryRadius = 0.4f;
	PxRigidDynamic* cameraCollider = NULL;
	PxRigidDynamic* CreateCamBoundary();
	void updateCameraCollider(PxRigidDynamic* cameraCollider, const PxVec3& cameraPosition);
	...
	void setTargetLocation(glm::vec3 target);
	...
	void setCameraRotation(float pitch, float yaw, float roll = 0.0f);
